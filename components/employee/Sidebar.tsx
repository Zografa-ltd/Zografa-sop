...
import { Department, DocumentSummary, ProductModel, parseDocumentType } from '@/lib/documents'
...
function SidebarInner({ departments, productModels, documents, isMobileOpen, onMobileClose }: SidebarProps) {
  const pathname = usePathname()
  const searchParams = useSearchParams()
  const { headings } = useToc()

  const activeDeptParam  = searchParams.get('dept')
  const activeModelParam = searchParams.get('model')
  ...
  // Render SOP links + type group links for a given set of docs + dept/model context
  function renderDocLinks(
    deptDocs: DocumentSummary[],
    deptId: string,
    modelId?: string,
  ) {
    ...
    return (
      <>
        ...
        {typeGroups.map((type) => {
          ...
          const isActive = activeDeptParam === deptId
            && activeTypeParam === type
            && (modelId ? activeModelParam === modelId : !activeModelParam)
          return (
            <Link
              key={type}
              href={href}
            ...
            >
              ...
              <span className="ml-auto flex-shrink-0 text-[10px] text-[#6B6660]">{count}</span>
            </Link>
          )
        })}
      </>
    ...
    >
      ...
      <nav className="sidebar-nav flex-1 overflow-y-auto px-2 py-3">
        ...
        {activeDepts.map((dept) => {
          ...
          // Show product model level only when dept has 2+ models
                ...
                <div className="mt-0.5 ml-2 space-y-0.5">
                  {useModelHierarchy
                    ? deptModels.map((model) => {
                        ...
                        const hasModelDocs = modelDocs.length > 0
                        return (
                          <div key={model.id}>
                            <p className={`px-2 pt-2 pb-1 text-[10px] font-semibold uppercase tracking-wider
                              ${hasModelDocs ? 'text-[#6B6660]' : 'text-[#C5C1BB]'}`}>
                              {model.name}
                            </p>
                            {hasModelDocs
                              ? renderDocLinks(modelDocs, dept.id, model.id)
                              : (
                                <p className="px-2 py-1 text-[10px] text-[#C5C1BB] italic">
                                  Няма документи
                                </p>
                              )
                            }
                    ...
                    : renderDocLinks(deptDocs, dept.id)
                  }
                </div>
              )}
            </div>
